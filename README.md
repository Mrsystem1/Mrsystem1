

<p align="center">
  <img src="https://typingsvg.vercel.app/api/svg?backgroundOpacity=0.98&lines=%5B%7B%22text%22%3A%22Hi+there%21+I%27m+Mr+System%5CnWeb+Penetration+Tester+%F0%9F%92%BB%5CnBug+Bounty+Hunter+%F0%9F%8E%AF%5CnActive+CTF+Player+%F0%9F%8F%81%22%2C%22color%22%3A%22%23e01b24%22%2C%22fontSize%22%3A25%2C%22typingSpeed%22%3A0.27322404371584696%7D%5D" alt="Typing SVG" />
</p>

### 🎯 About Me

I am an independent Ethical Hacker and Bug Bounty Hunter dedicated to web security and vulnerability discovery. I spend my time breaking down complex applications, hunting for security flaws, and solving challenges as a competitive CTF Player.

* **🌍 Location:** Dar es Salaam, Tanzania
* **✉️ Email:** cybermaster898@gmail.com
* **📞 Phone:** +255 659 644 768 / +255 795 242 8668
* **🚀 Mission:** Securing the web and climbing the leaderboards, one exploit at a time.

### 🛠️ Core Skills & Technologies

* **Programming & Scripting:** C, C++, Python, Assembly (x86/ARM), HTML
* **Web Security & Exploitation:** OWASP Top 10, Privilege Escalation, API Penetration Testing
* **Tools of the Trade:** Burp Suite, Nmap, Metasploit, Wireshark, Linux/Parrot OS
* **Focus Areas:** Web Vulnerability Assessment, Threat Modeling, Patch Verification

### 🏁 CTF & WarGames Domain Expertise
* **Categories:** Web Exploitation, Reverse Engineering, Binary Exploitation (Pwn), Digital Forensics



### 🏅 Certificates & Certifications

#### 🎯 CTF Competitions & Foundations

| Certificate | Platform / Issuer | Preview |
| :--- | :--- | :---: |
| SkillBit Flash CTF Certificate | **MetaCTF** | [📄 View Certificate](https://compete.metactf.com/641/certificate) |
| Certified Cybersecurity Foundations (CORE) | **Hackviser** | [📄 View Certificate](https://storage.hackviser.com/file/hackviser-prod/certificates/3884af197f844ff3919af127731a7735.pdf) |
| Course Completion: Aircrack-ng | **Just Hacking** | [📄 View Certificate](https://learn.justhacking.com/completion/60c4e1d47ae2640e) |
| Introduction to Cyber Security | **Simplilearn SkillUp** | [📄 View Certificate](https://certificates.simplicdn.net/share/10598548_10941284_1787879249373.pdf) |

#### 💀 Offensive Security & Red Teaming (Local Files)

| Certificate | Domain / Focus | Preview |
| :--- | :--- | :---: |
| Ethical Hacker | **Cisco** | [🔒 Local PDF File](file:///home/live/Downloads/EthicalHackerUpdate20260828-21-5fjz5q.pdf) |
| Red Team Leader (AV/EDR Evasion) | **Antivirus & EDR Bypass** | [🔒 Local PDF File](https://courses.redteamleaders.com/completion/c5ae3b1379815d27) |
| OPSEC & Anonymity | **Operational Security** | [🔒 Local PDF File](https://courses.redteamleaders.com/completion/43a6bf7cd7fe82a2) |

#### 🌐 Cisco (Academy Track)

| Certificate | Preview |
| :--- | :---: |
| Computer Hardware Basics | [📄 View Certificate](#) |
| Networking Basics | [📄 View Certificate](#) |
| Introduction to Cybersecurity | [📄 View Certificate](#) |

#### 📊 DataCamp

| Certificate | Preview |
| :--- | :---: |
| Data Science / Cybersecurity Basics | [📄 View Certificate](#) |
